...
define(function(require, exports, module) {
	...
	(function() {
		...
		// datepicker.js文件的dirname
		function L() {
			var _, A, $ = X[O][C]("script");
			for (var B = 0; B < $.length; B++) {
				_ = $[B].src.substring(0, $[B].src.toLowerCase().indexOf("datepicker.js"));
				A = _.lastIndexOf("/");
		...
		function U(J, C) {
			...
			if (C) {
				...
				var F = D();
				if (F) {
					$dp.srcEl = this.nodeType == 1 ? this : (F.srcElement || F.target);
					F.cancelBubble = true
				}
		...
		function K(H, $) {
			...
			if (!$dp.dd || $dp.eCont || ($dp.lang && $dp.realLang && $dp.realLang.name != $dp.lang && $dp.getLangIndex && $dp.getLangIndex($dp.lang) >= 0)) {
				...
				if (_.$dpPath == "") F(A);
				var datepickerUrl = '/public/moudlejs/datepicker/';	// 开发环境
				if(isDist()){	// 生产环境
					datepickerUrl = '/public/dist/moudlejs/datepicker/'
				}
				var B = "<iframe style=\"width:1px;height:1px\" src=\"" + datepickerUrl + "/My97DatePicker.html\" frameborder=\"0\" border=\"0\" scrolling=\"no\"></iframe>";
				if ($dp.eCont) {
	...
	function init97Date() {
		$("[datepicker]").each(function() {
			...
			if (!$this.data("initDatepicker")) {
				...
				if(options.startId){
					options['minDate'] = "#F{$dp.$D('" + options.startId + "')}";
				}
				
				if(options.endId){
					options['maxDate'] = "#F{$dp.$D('" + options.endId + "')}";
				}
				...
				$this.click(function(){
					WdatePicker.call(input, param);
					return false;
				});

				if($this.siblings('i.icon_time').length == 1){
					$this.siblings('i.icon_time').click(function(){
						WdatePicker.call(input, param);
						return false;
					});
				}
