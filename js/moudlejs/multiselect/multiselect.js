  ...
  (function(d) {
    ...
    d.widget("ech.multiselect", {
      ...
      _create: function() {
        ...
        if (b.singleSelect) {
          if (select.attr('valid')) {
            a.attr("valid", 'id@' + name)
          }
          this.element.removeAttr('valid');
        }
      ...
      refresh: function(a) {
        ...
        b.find("option").each(function(b) {
          ...
          if (c.singleSelect) { // 单选select
            e += '<input id="' + b + '" name="' + name + '" type="' + (c.multiple ? "checkbox" : "radio") + '" value="' + k + '" title="' + f + '"';
          } else {
...
//先择一个
  function initMultiselectOne($obj) {
    $.extend($.ech.multiselect.prototype.options, {
      noneSelectedText: '请选择',
      multiple:false,//去掉checkbox
      header:false,//去 掉全选与删除方法
      singleSelect: 1
    });
    var force = $obj  || false;
    var $obj = force ? $obj : $("select[multipleOne]");
    var defaultSetting = {
        selectedList: 1,
        classes: "custom"
    };
    ...
    $obj.each(function() {
      // if (!$(this).attr("initmultiselectone") || force) {
        var options = $(this).attr("multiselect-options");
        if (options) {
          options = $.parseJSON(options);
        }
        var setting = $.extend({}, defaultSetting, options);
        $(this).multiselect(setting);
        if (setting.defaultHide) {
          $(this).multiselect("getButton").hide();
        }
        $(this).attr("initmultiselectone", true);
      // }
    });
  }
  ...
  function initMultiselect($obj) {
    ...
    var force = $obj  || false;
    var $obj = force ? $obj : $("select[multiple]");
    var defaultSetting = {
        selectedList: 1,
        classes: "custom"     
    };
    ...
    $obj.each(function() {
      // if (!$(this).attr("initMultiselect") || force) {
        var options = $(this).attr("multiselect-options");
        if (options) {
          options = $.parseJSON(options);
        }
        ...
        $(this).attr("initMultiselect", true);
      // }
    });
  }
  ...
  exports.init = initMultiselect;
  exports.initOne = initMultiselectOne;
});